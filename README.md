...
이 프로젝트는 로컬 전투뿐 아니라 **실제 원격 접속 기반 1:1 온라인 대전**도 지원합니다.

- `GameEngine`이 게임 서버 역할을 하고, `MageFight` GUI 클라이언트가 여기에 접속합니다.
- 서버는 Cloudflare Tunnel(`cloudflared`)을 통해 `game.yeunsuh.online:9090`으로 노출됩니다.
- 플레이어는 로비에서 서버 주소를 직접 입력하지 않고, 닉네임과 캐릭터만 선택해 바로 매칭을 요청합니다.
- 두 플레이어가 연결되면 매치가 시작되고, 모든 액션은 JSON 프로토콜로 서버에 전달됩니다.
- 턴 종료 시점에 양쪽 액션이 한 번에 반영되어, 실시간 공정 대전을 목표로 합니다.

온라인 대전의 흐름은 다음과 같습니다.

1. 플레이어가 `MageFightApp`을 실행한다.
2. `Find Online Match`를 눌러 고정 서버(`game.yeunsuh.online:9090`)에 접속한다.
3. 닉네임과 캐릭터를 선택한 뒤 매칭을 요청한다.
4. 두 번째 플레이어가 접속하면 서버가 매치를 시작한다.
5. 전투 중에는 이동, 공격, 방어, 스킬 사용, 턴 종료가 서버를 통해 동기화된다.
